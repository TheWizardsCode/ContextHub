/**
 * Status and stage utility functions extracted from src/status-stage-rules.ts.
 *
 * Contains only pure utility functions with no CLI-specific config dependencies.
 * The CLI-specific `loadStatusStageRules()` and `createStatusStageRules()`
 * remain in the main src/status-stage-rules.ts.
 */
...
export function loadStatusStageRules(config?: WorklogConfig | null): StatusStageRules {
  ...
  return createStatusStageRules(config);
}
