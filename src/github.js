...
let _verboseLogger = null;
export function setVerboseLogger(logger) {
    ...
    while (true) {
        try {
            ...
            if (attempt < maxRetries && /403|rate limit/i.test(stderr + stdout)) {
                ...
                try {
                    throttler.incrementRetry && throttler.incrementRetry();
                }
                catch (_) { }
            ...
            if (stderr)
                e.message = `${e.message}\n${stderr}`;
            try {
                throttler.incrementError && throttler.incrementError();
            }
            catch (_) { }
            throw e;
        }
...
function runGhDetailed(command, input) {
...
// Async variants -----------------------------------------------------------
function spawnCommand(command, input, timeout = 120000) {
...
async function runGhAsync(command, input) {
    ...
    while (true) {
        ...
        if (attempt < maxRetries && /403|rate limit/i.test(stderr + stdout)) {
            ...
            try {
                logVerbose(`gh rate-limited (async spawn), retrying in ${waitMs}ms (attempt ${attempt + 1}/${maxRetries})`);
            }
            catch (_) { }
...
async function runGhDetailedAsync(command, input) {
    const res = await spawnCommand(command, input);
    if (res.code !== 0) {
...
async function runGhJsonDetailedAsync(command, input, retries = 3) {
    ...
    while (attempt <= maxRetries) {
        ...
        if (!res.ok) {
            ...
            if (attempt < maxRetries) {
                ...
                try {
                    logVerbose(`gh rate-limited/restricted, retrying in ${waitMs}ms (attempt ${attempt + 1}/${maxRetries})`);
                }
                catch (_) { }
                await new Promise(r => setTimeout(r, waitMs));
                attempt += 1;
        ...
        try {
            ...
            if (Array.isArray(data?.errors) && data.errors.length > 0) {
                const message = data.errors.map((entry) => entry?.message || String(entry)).join('; ');
...
async function runGhJsonAsync(command, input) {
    ...
    if (!detailed.ok)
        throw new Error(detailed.error || 'gh command failed');
    return detailed.data;
}
export async function ghApiAsyncScheduled(command, input) {
    return await throttler.schedule(async () => {
        return await runGhAsync(command, input);
    });
}
export async function ghApiDetailedScheduled(command, input) {
    return await throttler.schedule(async () => {
        return await runGhDetailedAsync(command, input);
    });
}
export async function ghApiJsonScheduled(command, input) {
    return await throttler.schedule(async () => {
        return await runGhJsonAsync(command, input);
    });
}
function runGhSafe(command, input) {
    try {
...
function runGhJson(command, input) {
    const output = runGh(command, input);
    return JSON.parse(output);
}
export function isSecondaryRateLimitText(text) {
    if (!text)
        return false;
...
function runGhJsonWithRetries(command, input, retries = 3) {
    const res = runGhJsonDetailed(command, input);
    if (!res.ok)
        throw new Error(res.error || 'gh command failed');
    return res.data;
}
function runGhSafeJson(command, input) {
...
function runGhJsonDetailed(command, input, retries = 3) {
 ...
 */
const SINGLE_VALUE_LABEL_CATEGORIES = ['status:', 'priority:', 'stage:', 'type:', 'risk:', 'effort:'];
function isStatusLabel(label, labelPrefix) {
 ...
 * issue.
 */
export function isSingleValueCategoryLabel(label, labelPrefix) {
    const normalizedPrefix = normalizeGithubLabelPrefix(labelPrefix);
    if (!label.startsWith(normalizedPrefix)) {
        return false;
    }
 ...
 */
function ensureGithubLabels(config, labels) {
    ...
    if (existing === undefined && !existingLabelsCache.has(config.repo)) {
        ...
        if (existingRaw) {
            ...
            existingLabelsCache.set(config.repo, new Set());
...
export function normalizeGithubLabelPrefix(prefix) {
    if (!prefix)
        return 'wl:';
...
export function extractWorklogId(body) {
    if (!body)
        return null;
...
export function extractWorklogCommentId(body) {
    if (!body)
        return null;
...
export function extractParentId(body) {
    if (!body)
        return null;
    const lines = body.split('\n');
...
export function extractParentIssueNumber(body) {
    if (!body)
        return null;
    const lines = body.split('\n');
    for (const line of lines) {
...
function getIssueNodeId(config, issueNumber) {
    const { owner, name } = parseRepoSlug(config.repo);
    ...
    const output = runGhJsonDetailed(`gh api graphql -f query=${quoteShellValue(query)} -f owner=${quoteShellValue(owner)} -f name=${quoteShellValue(name)} -F number=${issueNumber}`);
    if (!output.ok) {
...
export function getIssueHierarchy(config, issueNumber) {
    const { owner, name } = parseRepoSlug(config.repo);
    const query = `query($owner: String!, $name: String!, $number: Int!) {
    repository(owner: $owner, name: $name) {
      issue(number: $number) {
    ...
    const childIssueNumbers = Array.isArray(issue?.subIssues?.nodes)
        ? issue.subIssues.nodes.map((node) => node?.number).filter((value) => typeof value === 'number')
        : [];
    return { parentIssueNumber, childIssueNumbers };
}
// Async wrappers -----------------------------------------------------------
export async function getIssueNodeIdAsync(config, issueNumber) {
    const { owner, name } = parseRepoSlug(config.repo);
    const query = `query($owner: String!, $name: String!, $number: Int!) { repository(owner: $owner, name: $name) { issue(number: $number) { id } } }`;
    const data = await ghApiJsonScheduled(`gh api graphql -f query=${quoteShellValue(query)} -f owner=${quoteShellValue(owner)} -f name=${quoteShellValue(name)} -F number=${issueNumber}`);
...
export async function getIssueHierarchyAsync(config, issueNumber) {
    const { owner, name } = parseRepoSlug(config.repo);
    ...
    const result = await ghApiJsonScheduled(`gh api graphql -f query=${quoteShellValue(query)} -f owner=${quoteShellValue(owner)} -f name=${quoteShellValue(name)} -F number=${issueNumber}`);
    if (!result)
        throw new Error('Unable to query issue hierarchy');
...
export function addSubIssueLink(config, parentIssueNumber, childIssueNumber, cache) {
    const nodeCache = cache ?? new Map();
    const resolveNodeId = (issueNumber) => {
    ...
    const result = runGhJsonDetailed(`gh api graphql -f query=${quoteShellValue(mutation)} -f parent=${quoteShellValue(parentNodeId)} -f child=${quoteShellValue(childNodeId)}`);
    if (!result.ok) {
...
export function addSubIssueLinkResult(config, parentIssueNumber, childIssueNumber, cache) {
    try {
...
export async function addSubIssueLinkAsync(config, parentIssueNumber, childIssueNumber, cache) {
    const nodeCache = cache ?? new Map();
    const resolveNodeId = async (issueNumber) => {
        const cached = nodeCache.get(issueNumber);
        if (cached)
            return cached;
    ...
    const result = await throttler.schedule(async () => {
        return await runGhJsonDetailedAsync(`gh api graphql -f query=${quoteShellValue(mutation)} -f parent=${quoteShellValue(parentNodeId)} -f child=${quoteShellValue(childNodeId)}`);
    });
    if (!result.ok) {
        throw new Error(result.error || `Failed to link #${childIssueNumber} as sub-issue of #${parentIssueNumber}`);
    }
...
export async function addSubIssueLinkResultAsync(config, parentIssueNumber, childIssueNumber, cache) {
    try {
...
export function listParentIssueNumbersFromTimeline(config, issueNumber) {
...
export function extractChildIds(body) {
    if (!body)
        return [];
    const lines = body.split('\n');
    const childIds = [];
...
export function extractChildIssueNumbers(body) {
    if (!body)
        return [];
    const lines = body.split('\n');
...
export function workItemToIssuePayload(item, comments, labelPrefix, allItems) {
    const marker = buildWorklogMarker(item.id);
    const summaryLines = [marker];
    if (allItems) {
        void allItems;
    }
    summaryLines.push('');
 ...
 */
export function listGithubIssueComments(config, issueNumber) {
    const { owner, name } = parseRepoSlug(config.repo);
...
// Async variants -----------------------------------------------------------
export async function listGithubIssueCommentsAsync(config, issueNumber) {
    const { owner, name } = parseRepoSlug(config.repo);
    const command = `gh api repos/${owner}/${name}/issues/${issueNumber}/comments --paginate`;
    try {
 ...
 */
export function createGithubIssueComment(config, issueNumber, body) {
    const { owner, name } = parseRepoSlug(config.repo);
...
export async function createGithubIssueCommentAsync(config, issueNumber, body) {
 ...
 */
export function updateGithubIssueComment(config, commentId, body) {
    const { owner, name } = parseRepoSlug(config.repo);
    const command = `gh api -X PATCH repos/${owner}/${name}/issues/comments/${commentId} -F body=@-`;
    const data = runGhJson(command, body);
    return normalizeGithubIssueComment(data);
}
export async function updateGithubIssueCommentAsync(config, commentId, body) {
 ...
 */
export function getGithubIssueComment(config, commentId) {
    const { owner, name } = parseRepoSlug(config.repo);
    const command = `gh api repos/${owner}/${name}/issues/comments/${commentId} --json id,body,updatedAt,user`;
    const data = runGhJson(command);
    return normalizeGithubIssueComment(data);
}
export async function getGithubIssueCommentAsync(config, commentId) {
    const { owner, name } = parseRepoSlug(config.repo);
    const command = `gh api repos/${owner}/${name}/issues/comments/${commentId} --json id,body,updatedAt,user`;
 ...
 */
export async function assignGithubIssueAsync(config, issueNumber, assignee, retries = 3) {
    let attempt = 0;
    let backoff = 500;
    while (attempt <= retries) {
        // Schedule assignment through the throttler to respect concurrency/rate limits.
        const res = await throttler.schedule(async () => {
            return await runGhDetailedAsync(`gh issue edit ${issueNumber} --repo ${config.repo} --add-assignee ${JSON.stringify(assignee)}`);
        });
        if (res.ok) {
            return { ok: true };
        }
 ...
 * without throwing.
 */
export function assignGithubIssue(config, issueNumber, assignee) {
    ...
    const res = runGhDetailed(`gh issue edit ${issueNumber} --repo ${config.repo} --add-assignee ${JSON.stringify(assignee)}`);
    if (res.ok) {
        return { ok: true };
    }
...
export function issueToWorkItemFields(issue, labelPrefix) {
    const normalizedPrefix = normalizeGithubLabelPrefix(labelPrefix);
    const tags = [];
    let status = issue.state === 'closed' ? 'completed' : 'open';
    let priority = 'medium';
    let risk = '';
    let effort = '';
    let stage = '';
 ...
 */
export function createGithubIssue(config, payload) {
...
export async function ensureGithubLabelsAsync(config, labels) {
    const unique = Array.from(new Set(labels.filter(label => label.trim() !== '')));
    if (unique.length === 0)
        return;
    const { owner, name } = parseRepoSlug(config.repo);
    try {
        ...
        for (const label of unique) {
            ...
            try {
                ...
                const fallbackCommand = `gh issue label create ${JSON.stringify(label)} --repo ${config.repo} --color ${color}`;
                try {
                    await ghApiAsyncScheduled(fallbackCommand);
                    existing.add(label);
                }
                catch (_) { /* ignore */ }
            }
...
const ensuredLabelsCache = new Map();
...
const existingLabelsCache = new Map();
/**
 ...
 */
function ensureGithubLabelsOnce(config, labels) {
    const unique = Array.from(new Set(labels.filter(label => label.trim() !== '')));
    if (unique.length === 0)
        return;
...
async function ensureGithubLabelsOnceAsync(config, labels) {
    const unique = Array.from(new Set(labels.filter(label => label.trim() !== '')));
    if (unique.length === 0)
        return;
    const cacheKey = config.repo;
...
export async function createGithubIssueAsync(config, payload) {
    return await throttler.schedule(async () => {
        const command = `gh issue create --repo ${config.repo} --title ${JSON.stringify(payload.title)} --body-file -`;
        ...
        if (issueNumber !== null && payload.labels.length > 0) {
            ...
            try {
                await runGhAsync(`gh issue edit ${issueNumber} --repo ${config.repo} --add-label ${JSON.stringify(payload.labels.join(','))}`);
            }
            catch (_) { }
        }
        if (issueNumber === null) {
            ...
            if (Array.isArray(view) && view.length > 0)
                return normalizeGithubIssue(view[0]);
            throw new Error('Failed to create GitHub issue');
        }
...
export async function updateGithubIssueAsync(config, issueNumber, payload) {
...
export async function getGithubIssueAsync(config, issueNumber) {
...
export async function listGithubIssuesAsync(config, since) {
 ...
 */
export function updateGithubIssue(config, issueNumber, payload) {
 ...
 */
export function listGithubIssues(config, since) {
    const sinceParam = since ? `&since=${encodeURIComponent(since)}` : '';
    const apiPath = `repos/${config.repo}/issues?state=all&per_page=100${sinceParam}`;
    const apiCommand = `gh api ${quoteShellValue(apiPath)} --paginate`;
 ...
 */
export function getGithubIssue(config, issueNumber) {
...
function normalizeGithubIssue(raw) {
    ...
    return {
        ...
        labels: Array.isArray(raw.labels) ? raw.labels.map((l) => l.name || l) : [],
 ...
 */
export async function fetchLabelEventsAsync(config, issueNumber, cache) {
 ...
 */
export function labelFieldsDiffer(labelFields, localItem) {
    if (labelFields.status !== localItem.status)
        return true;
    if (labelFields.priority !== localItem.priority)
        return true;
    if (labelFields.stage && labelFields.stage !== localItem.stage)
        return true;
    if (labelFields.issueType && labelFields.issueType !== localItem.issueType)
        return true;
    if (labelFields.risk && labelFields.risk !== localItem.risk)
        return true;
    if (labelFields.effort && labelFields.effort !== localItem.effort)
        return true;
    return false;
}
/**
 ...
 */
export function getLatestLabelEventTimestamp(events, labelPrefix, category) {
    const normalizedPrefix = normalizeGithubLabelPrefix(labelPrefix);
