 ...
 */
export function resolveWlBinary(explicit?: string): string {
 ...
 */
function generateAuditReport(item: Record<string, any>, id: string): string {
  ...
  const lines: string[] = [
    ...
    children.length > 0
      ? children
          .map(
            (c: any) =>
              `  ${c.id}: ${c.title} [${c.status}] - ${c.stage || "no stage"}`
          )
          .join("\n")
      : "  None",
    ``,
    ...
    comments.length > 0
      ? comments
          .map((c: any, i: number) => `  C${i + 1}: ${c.author} - ${c.comment?.substring(0, 200) || "(no comment)"}`)
          .join("\n")
      : "  None",
    ``,
  ];
 ...
 */
export async function runPiAudit(
  options: RunPiAuditOptions
): Promise<RunPiAuditResult> {
 ...
 */
export function isWaitingForInputEvent(_event: unknown): boolean {
 ...
 */
export async function runAudit(options: RunPiAuditOptions): Promise<RunPiAuditResult> {
