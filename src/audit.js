 ...
 */
export function redactAuditText(auditText) {
    if (!auditText)
        return auditText;
    // Match local@domain.tld where domain contains at least one dot and TLD-like tail
    const emailRe = /([A-Za-z0-9._%+-]+)@([A-Za-z0-9.-]+\.[A-Za-z]{2,})/g;
    return auditText.replace(emailRe, (_match, local, domain) => {
        const first = local && local.length > 0 ? local[0] : '';
        return `${first}***@${domain}`;
    });
}
export function hasAcceptanceCriteria(description) {
    if (!description)
        return false;
...
export function buildAuditEntry(auditText, author, opts) {
...
export function inspectAuditFirstLine(auditText) {
...
export function formatInvalidAuditFirstLineMessage(inspection) {
 ...
 */
export function parseReadinessLine(auditText) {
    const inspection = inspectAuditFirstLine(auditText);
    if (inspection.trimmedFirstNonEmptyLine === READY_TO_CLOSE_YES)
        return 'Complete';
    if (inspection.trimmedFirstNonEmptyLine === READY_TO_CLOSE_NO)
        return 'Partial';
