...
async function gitExecAsync(command, options) {
...
async function execGitCaptureStdout(args, options) {
 ...
 */
export function mergeWorkItems(localItems, remoteItems, options) {
    const conflicts = [];
...
function mergeRemoteItem(mergedMap, remoteItem, options, conflicts, conflictDetails) {
 ...
 */
export function isOwnProjectItemId(id, projectPrefix) {
 ...
 */
export function filterRemoteDataByPrefix(items, comments, edges, audits, projectPrefix) {
...
function mergeSameTimestampItems(localItem, remoteItem, options) {
    ...
    const fields = ['title', 'description', 'status', 'priority', 'sortIndex', 'parentId', 'tags', 'assignee', 'stage', 'issueType', 'createdBy', 'deletedBy', 'deleteReason'];
    const mergedFields = [];
    const fieldDetails = [];
...
function mergeDifferentTimestampItems(localItem, remoteItem, options) {
    ...
    if (conflictedFields.length > 0) {
        conflictMessages.push(`${remoteItem.id}: Conflicting fields [${conflictedFields.join(', ')}] resolved using ${isRemoteNewer ? 'remote' : 'local'} values (${isRemoteNewer ? 'remote' : 'local'}: ${isRemoteNewer ? remoteItem.updatedAt : localItem.updatedAt}, ${isRemoteNewer ? 'local' : 'remote'}: ${isRemoteNewer ? localItem.updatedAt : remoteItem.updatedAt})`);
    }
    if (mergedFields.length > 0) {
        conflictMessages.push(`${remoteItem.id}: Merged fields [${mergedFields.join(', ')}]`);
    }
 ...
 */
export function mergeAuditResults(localAudits, remoteAudits) {
    const mergedMap = new Map();
 ...
 */
export function mergeDependencyEdges(localEdges, remoteEdges) {
 ...
 */
async function getRepoRootForPath(filePath) {
 ...
 */
export async function assertDataFileInCwdRepo(dataFilePath) {
    let cwdRepoRoot = null;
    try {
    ...
    if (cwdRepoRoot && dataRepoRoot && cwdRepoRoot !== dataRepoRoot) {
        throw new Error(`Cross-project sync blocked: data file '${dataFilePath}' belongs to git repository '${dataRepoRoot}', ` +
    ...
    if (cwdRepoRoot && !dataRepoRoot) {
        throw new Error(`Cross-project sync blocked: data file '${dataFilePath}' is not inside a git repository, ` +
            `but this command is running inside git repository '${cwdRepoRoot}'. ` +
...
function getRemoteTrackingRef(remote, branchOrRef) {
...
async function fetchTargetRef(target) {
    ...
    if (target.branch.startsWith('refs/')) {
        ...
        try {
            ...
            try {
                const { stdout } = await execAsync(`git ls-remote --exit-code ${escapeShellArg(target.remote)} ${escapeShellArg(target.branch)}`);
...
function getRepoRelativePath(repoRootPath, filePath) {
    const absolutePath = path.resolve(filePath);
    const relativePath = path.relative(repoRootPath, absolutePath);
    return { absolutePath, relativePath };
}
export async function getRemoteDataFileContent(dataFilePath, target) {
...
function removeWorktreeFiles(worktreePath) {
...
async function listTrackedFiles(worktreePath) {
...
async function withTempWorktree(repoRootPath, target, run, options) {
        ...
        try {
            ...
            if (baseRef === 'HEAD') {
                ...
                if (/HEAD/.test(raw) && /(not a commit|invalid reference|cannot be created from|not a valid object name)/i.test(raw)) {
                    throw new Error('Cannot sync: this repository has no commits yet, so git cannot create ' +
...
export async function gitPushDataFileToBranch(repoDataFilePath, commitMessage, target) {
    ...
    if (branch.startsWith('refs/heads/') || branch.startsWith('refs/tags/')) {
        throw new Error(`Refusing to push worklog data to '${branch}'. ` +
    ...
    await withTempWorktree(repoRootPath, target, async (worktreePath) => {
        ...
        const { stdout: staged } = await execAsync(`git -C ${escapeShellArg(worktreePath)} diff --cached --name-only -- ${escapedRel}`);
        if (!staged.trim()) {
            return;
        }
        ...
        await execAsync(`git -C ${escapeShellArg(worktreePath)} push --no-verify ${escapeShellArg(target.remote)} HEAD:${escapeShellArg(pushTarget)}`);
    });
}
/**
 ...
 */
export async function rewriteAndForcePushDataFile(repoDataFilePath, commitMessage, target) {
    ...
    await withTempWorktree(repoRootPath, target, async (worktreePath) => {
        ...
        await execAsync(`git -C ${escapeShellArg(worktreePath)} push --force --no-verify ${escapeShellArg(target.remote)} HEAD:${escapeShellArg(pushTarget)}`);
