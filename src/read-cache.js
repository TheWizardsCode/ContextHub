...
export function dbFilesForWorklogDir(worklogDir) {
    return {
 ...
 */
export function computeDbFingerprint(worklogDir) {
...
export function stateCounterFilePath(cacheDir, worklogDir) {
 ...
 */
export function readStateCounter(cacheDir, worklogDir) {
    try {
 ...
 */
export function bumpStateCounter(cacheDir, worklogDir) {
    ...
    try {
        ...
        debugLog(`failed to bump state counter: ${err.message}`);
    }
    return next;
}
/**
 ...
 */
export function counterFingerprint(cacheDir, worklogDir) {
 ...
 */
export function deriveCacheKey(worklogDir, argv, version) {
...
export class ReadCache {
     ...
     */
    set(worklogDir, argv, value, options) {
        const dir = path.resolve(worklogDir);
        const key = this.keyFor(dir, argv);
        const now = this.now();
        const entry = {
    ...
    safeAtomicWrite(target, entry) {
        try {
            this.atomicWrite(target, entry);
        }
        catch (err) {
            debugLog(`failed to touch ${target}: ${err.message}`);
        }
    }
    ...
    removeFile(p) {
        try {
            ...
            if (err && err.code === 'ENOENT')
                return false;
            debugLog(`failed to remove ${p}: ${err.message}`);
            return false;
        }
    }
    /**
