...
const MIGRATIONS = [
    {
        ...
        apply: (db) => {
            const cols = db.prepare(`PRAGMA table_info('workitems')`).all();
    ...
    },
    {
        ...
        apply: (db) => {
            ...
            const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='audit_results'").all();
            if (tables.length === 0) {
                db.exec(`
    ...
    },
    {
        ...
        apply: (db) => {
            ...
            try {
                ...
                if (metaRow && String(metaRow.value) === '1') {
                    alreadyDone = true;
                }
            }
            ...
            const rows = db.prepare('SELECT id, audit FROM workitems WHERE audit IS NOT NULL AND audit != \'\'').all();
...
export function listPendingMigrations(dbPath) {
    ...
    try {
        ...
        let existingMeta;
        try {
            const metaRows = db.prepare('SELECT key FROM metadata').all();
...
function makeBackup(dbPath, logger) {
    ...
    for (let i = keep; i < files.length; i += 1) {
        try {
            ...
            logger?.error?.(`Failed to prune old backup ${files[i].full}: ${err.message}`);
        }
    }
    logger?.info?.(`Created backup: ${out}`);
    return out;
}
export function runMigrations(opts = {}, dbPath, filter) {
    const file = resolveDbPath(dbPath);
    ...
    try {
        ...
        throw new Error(`Failed to create backup before applying migrations: ${err.message}`);
    }
    ...
    try {
        const tx = db.transaction(() => {
            ...
            try {
                ...
                logger.error?.(`Failed to update metadata.schemaVersion: ${err.message}`);
            }
        });
        tx();
    }
