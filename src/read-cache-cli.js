 ...
 */
import { ReadCache, bumpStateCounter, counterFingerprint, fingerprintsEqual, resolveCacheDir, } from './read-cache.js';
...
export const READ_CACHE_COMMANDS = new Set(['list', 'next', 'show', 'search', 'status']);
/**
 ...
 */
export function shouldCacheReadInvocation(command, argv) {
    if (!isCacheableReadCommand(command))
        return false;
    if (!argvIsJsonMode(argv))
        return false;
    if (hasNonCacheableReadFlags(argv))
        return false;
    return true;
}
/** Canonical argv for cache keys: process.argv minus the node + script prefix. */
export function cacheKeyArgv() {
 ...
 * (bare `wl`, `wl --json`, etc.).
 */
export function extractCommandFromArgv(argv) {
...
export class ReadCacheCli {
    ...
    onJsonOutput(data) {
        ...
        this.backfill = null;
        if (b === null || data === null || typeof data !== 'object')
            return;
        const fpNow = this.cache.fingerprintFor(b.worklogDir);
        if (!fingerprintsEqual(fpNow, b.fpBefore))
            return;
        try {
...
//# sourceMappingURL=read-cache-cli.js.map