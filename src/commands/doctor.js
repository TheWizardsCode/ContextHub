...
import { dryRunHooks, upgradeHooks, detectHooksTargetDir } from '../doctor/hook-upgrade.js';
...
export default function register(ctx) {
    ...
    doctor
        ...
        try {
            ...
            if (utils.isJsonMode()) {
                ...
                try {
                    let migrationResult = { applied: [], backups: [] };
                    if (hasPendingMigrations) {
            ...
            if (totalItems > 0 && !proceed) {
                ...
                const answer = await new Promise(resolve => {
                    ...
                    const parts = [];
                    if (hasPendingMigrations)
                        parts.push(`${pending.length} pending migration(s)`);
                    if (hasOutdatedHooks)
                        parts.push(`${hooksResult.outdatedCount} outdated hook(s)`);
                    rl.question(`Apply ${parts.join(' and ')}? (y/N): `, (a) => {
                        rl.close();
            ...
            if (!proceed) {
                if (utils.isJsonMode())
                    output.json({ success: false, message: 'User declined to apply changes' });
                else
                    console.log('Aborted: changes not applied.');
                return;
            }
            // Apply hooks first, then migrations
            let hooksApplied = [];
            ...
            try {
                ...
                if (migrationResult.backups && migrationResult.backups.length > 0)
                    console.log(`Backups: ${migrationResult.backups.join(', ')}`);
            ...
            if (utils.isJsonMode())
                ...
                console.error(`Doctor upgrade failed: ${message}`);
        }
    });
    doctor
        .command('stage-sync')
    ...
    doctor
        ...
        try {
                ...
                try {
                    if (storeAny && typeof storeAny.deleteWorkItem === 'function') {
                        ...
                        if (ok) {
                            ...
                            try {
                                storeAny.deleteDependencyEdgesForItem(id);
                            }
                            catch (_) { }
                            pruned.push(id);
                        }
                        ...
                        const ok = await Promise.resolve(db.delete(id));
                        if (ok)
                            pruned.push(id);
                    }
                    else {
            ...
            if (utils.isJsonMode())
                ...
                console.error(`Doctor prune failed: ${message}`);
        }
    });
    doctor
        ...
        if (opts.apply) {
            ...
            if (opts.push) {
                ...
                pushResult = await withFileLock(lockPath, async () => {
                    const tipSha = await rewriteAndForcePushDataFile(jsonlPath, 'Rewrite worklog ref: remove foreign items', gitTarget);
    ...
    doctor
        ...
        if (opts.dryRun || !opts.apply) {
            if (utils.isJsonMode()) {
                const out = { dryRun: true, invalid, count: invalid.length };
                if (!opts.dryRun)
                    out.hint = 'Use --apply to fix invalid priorities';
                output.json(out);
                return;
            }
        ...
        for (const entry of invalid) {
            if (entry.mapped) {
                try {
                    db.update(entry.id, { priority: entry.mapped });
    ...
    doctor
        ...
        const doFix = (finding) => {
            if (!applyFilePathsFix(finding, (_id, _updates) => true)) {
                return false;
            }
            ...
            if (!item)
                return false;
            const appendText = finding.proposedFix?.appendDescription;
            if (!appendText)
                return false;
        ...
        const intakeItemCount = allItems.filter(i => DEFAULT_INTAKE_STAGES.includes(i.stage) && i.status !== 'deleted').length;
        if (opts.addPlaceholder) {
            ...
            if (utils.isJsonMode()) {
                output.json({
                    ...
                    missing: missing.map(f => ({ itemId: f.itemId, title: f.context?.itemTitle })),
                    withIncorrect: withIncorrect.map(f => ({ itemId: f.itemId, title: f.context?.itemTitle })),
                    fixed: fixed.length,
                    fixedItems: fixed,
                });
                return;
        ...
        if (utils.isJsonMode()) {
            output.json({
                success: true,
                total: intakeItemCount,
                missing: missing.map(f => ({ itemId: f.itemId, title: f.context?.itemTitle, message: f.message })),
                withIncorrect: withIncorrect.map(f => ({ itemId: f.itemId, title: f.context?.itemTitle, message: f.message })),
            });
            return;
        }
...
${missing.length} item(s) missing **Key Files:** section:`);
            for (const f of missing) {
                console.log(`  - ${f.itemId}: ${f.context?.itemTitle || f.itemId}`);
            }
        }
...
${withIncorrect.length} item(s) with incorrect **Key Files:** sections:`);
    ...
    doctor.action(async (options) => {
        ...
        for (const item of items) {
            ...
            if (p && !isValidPriority(p)) {
                ...
                priorityFindings.push({
                    ...
                    proposedFix: mapped ? { priority: mapped } : null,
                    safe: !!mapped,
                    context: { current: p, mapped },
                });
            }
        }
        let findings = [
        ...
        if (options.fix) {
            ...
            for (const f of findings) {
                try {
                    ...
                    if (f.type === 'incompatible-status-stage' && ctx.status === 'completed' && (ctx.stage === 'in_progress' || ctx.stage === 'intake_complete' || ctx.stage === 'idea')) {
                        const current = (f.proposedFix && typeof f.proposedFix === 'object') ? f.proposedFix : {};
                        f.proposedFix = Object.assign({}, current, { stage: 'in_review' });
                        f.safe = true;
                    }
                    // deleted + in_progress -> deleted + done
                    if (f.type === 'incompatible-status-stage' && ctx.status === 'deleted' && ctx.stage === 'in_progress') {
                        const current = (f.proposedFix && typeof f.proposedFix === 'object') ? f.proposedFix : {};
                        f.proposedFix = Object.assign({}, current, { stage: 'done' });
                        f.safe = true;
                    }
                }
                catch (e) {
            ...
            for (const f of findings) {
                try {
                    if (f.type === 'invalid-stage' && f.context && f.context.stage === '') {
                        const current = (f.proposedFix && typeof f.proposedFix === 'object') ? f.proposedFix : {};
            ...
            for (const f of findings) {
                if (f.safe && f.proposedFix && typeof f.proposedFix === 'object') {
                    try {
                        ...
                        const update = {};
                        if (f.proposedFix.status)
                            update.status = f.proposedFix.status;
                        if (f.proposedFix.stage)
                            update.stage = f.proposedFix.stage;
                        if (f.proposedFix.priority)
                            update.priority = f.proposedFix.priority;
            ...
            const finalFindings = [];
            const readlineMod = await import('node:readline');
            const promptInteractive = (promptText) => {
                const rl = readlineMod.createInterface({ input: process.stdin, output: process.stdout });
            ...
            for (const f of remainingFindings) {
                ...
                const hasActionableFix = f.proposedFix && typeof f.proposedFix === 'object' && (Object.prototype.hasOwnProperty.call(f.proposedFix, 'status') ||
                ...
                if (!hasActionableFix) {
                    ...
                    try {
                        f.context = { ...(f.context || {}), requiresManualFix: true };
                    }
                    catch (e) { }
                    finalFindings.push(f);
                    continue;
                    ...
                    try {
                        ...
                        if (item) {
                            ...
                            if (Object.keys(update).length > 0) {
                                ...
                                catch (err) { /* fall through to keep in report */ }
                            }
                        }
                    }
                    catch (err) {
        ...
        const manual = findings.filter(f => {
            const ctx = f.context || {};
            const proposed = f.proposedFix;
            const hasActionableFix = proposed && typeof proposed === 'object' && (Object.prototype.hasOwnProperty.call(proposed, 'status') ||
        ...
        if (manual.length > 0) {
            ...
            for (const [type, group] of byType.entries()) {
                ...
                for (const f of group) {
                    ...
                    const suggestions = [];
                    if (proposed) {
                        if (proposed.allowedStages)
                            suggestions.push(`allowedStages=${JSON.stringify(proposed.allowedStages)}`);
                        if (proposed.allowedStatuses)
                            suggestions.push(`allowedStatuses=${JSON.stringify(proposed.allowedStatuses)}`);
                        if (proposed.stage)
                            suggestions.push(`proposedStage=${String(proposed.stage)}`);
                        if (proposed.status)
                            suggestions.push(`proposedStatus=${String(proposed.status)}`);
                        if (proposed.priority)
                            suggestions.push(`proposedPriority=${String(proposed.priority)}`);
                    }
