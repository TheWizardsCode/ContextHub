...
function normalizeBooleanOption(value, flagName) {
    if (value === undefined)
        return undefined;
...
function normalizeAgentTemplateAction(value) {
    if (!value)
        return undefined;
    const normalized = value.trim().toLowerCase();
    if (normalized === 'overwrite' || normalized === 'o')
        return 'overwrite';
    if (normalized === 'append' || normalized === 'a')
        return 'append';
    if (normalized === 'skip' || normalized === 'm' || normalized === 'manual' || normalized === 'manage')
        return 'skip';
...
function ensureGitignore(options) {
    ...
    try {
        ...
        return { updated: false, present: false, gitignorePath, reason: e.message };
    }
    ...
    try {
        ...
        return { updated: false, present: fs.existsSync(gitignorePath), gitignorePath, reason: e.message };
    }
...
function installPrePushHook(options) {
    try {
    ...
    try {
        ...
        return { installed: false, skipped: true, present: false, hookPath: hookFile, reason: e.message };
    }
}
function installPostPullHooks(options) {
    try {
        execSync('git rev-parse --is-inside-work-tree', { stdio: 'ignore' });
    }
    catch {
    ...
    const centralScriptContent = `#!/bin/sh\n` +
    ...
    const wrapperContent = (centralPath) => (`#!/bin/sh\n` +
        `# ${WORKLOG_POST_PULL_HOOK_MARKER}\n` +
    ...
    try {
        ...
        if (fs.existsSync(centralScript)) {
            ...
            try {
                fs.chmodSync(centralScript, 0o755);
            }
            catch { }
        }
        ...
        return { installed: false, skipped: true, present: false, hookPaths: hookFiles, reason: e.message };
    }
}
function installCommittedHooks(options) {
    ...
    try {
        ...
        return { installed: false, skipped: true, present: false, dirPath: dir, files: hookFiles, reason: e.message };
    }
}
function getSyncDefaults(config) {
    return {
...
async function ensureWorkflowTemplateInstalled(options) {
    ...
    if (!templatePath)
        return { installed: false, skipped: true, reason: 'workflow template not found', templatePath: null, destinationPath: null };
    const projectRoot = resolveProjectRoot();
    ...
    if (options.agentDestinationPath) {
        try {
            ...
            if (insertResult.inserted) {
                if (!options.silent)
                    console.log(`✓ Inlined WORKFLOW content into ${options.agentDestinationPath}`);
                return { installed: true, skipped: false, templatePath, destinationPath: null };
            }
            // Already present is not an error — report as skipped
            if (insertResult.reason === 'already present')
                return { installed: false, skipped: true, reason: 'already in place', templatePath, destinationPath: null };
            return { installed: false, skipped: true, reason: insertResult.reason || 'insertion failed', templatePath, destinationPath: null };
        }
        catch (e) {
            return { installed: false, skipped: true, reason: e.message, templatePath, destinationPath: null };
        }
    }
...
function insertWorkflowLoaderIntoAgents(agentPath) {
    try {
        if (!fs.existsSync(agentPath))
            return { inserted: false, reason: 'agents file not found' };
        ...
        if (!workflowContent)
            return { inserted: false, reason: 'workflow file not found' };
        ...
        if (existing.includes('<!-- WORKFLOW: start -->'))
        ...
        return { inserted: false, reason: e.message };
    }
}
...
async function ensureStatsPluginInstalled(options) {
    ...
    try {
        ...
        if (fs.existsSync(destinationPath)) {
            ...
            if (already)
                return { installed: false, skipped: true, reason: 'already in place', templatePath, destinationPath };
            if (options.overwrite === false)
                return { installed: false, skipped: true, reason: 'user declined', templatePath, destinationPath };
        ...
        return { installed: false, skipped: true, reason: e.message, templatePath, destinationPath };
    }
}
function resolveAgentDestination(projectRoot) {
    return path.join(projectRoot, WORKLOG_AGENT_DESTINATION_FILENAME);
}
function normalizeContent(content) {
    return content.replace(/\r\n/g, '\n').trimEnd();
}
function analyzeAgentContent(existingContent) {
...
async function promptAgentTemplateAction() {
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    return new Promise(resolve => {
        const prompt = 'AGENTS.md already exists. To use Worklog we need to ensure that agents are aware of it. You have a few options:\n\n' +
...
async function ensureAgentTemplateInstalled(options) {
...
function printAgentTemplateSummary() {
}
async function performInitSync(dataPath, prefix, isJsonMode = false) {
    ...
    const db = new (await import('../database.js')).WorklogDatabase(prefix || config?.prefix || 'WL', undefined, dataPath);
            ...
            if (isJsonMode) {
                ...
                try {
                    ...
                    if (postPullResult && postPullResult.installed) {
                        ...
                        if (postPullResult.centralScriptPath) {
                            console.log(`Git post-pull hooks: installed (central script at ${postPullResult.centralScriptPath})`);
                        }
                        else {
                    ...
                    if (workflowTemplatePath) {
                        ...
                        if (fs.existsSync(agentDestination)) {
                            ...
                            if (agentContent.includes('<!-- WORKFLOW: start -->')) {
                                ...
                                const choice = await promptWorkflowChoice(workflowInlineProvided ? normalizedOptions.workflowInline : undefined);
                                if (choice === 'basic') {
                    ...
                    output.error('Error: ' + error.message, { success: false, error: error.message });
                    process.exit(1);
                }
            }
        }
