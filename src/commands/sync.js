...
export function getSyncDefaults(config) {
    return {
        gitRemote: config?.syncRemote || DEFAULT_GIT_REMOTE,
        gitBranch: config?.syncBranch || DEFAULT_GIT_BRANCH,
    };
}
export async function performSync(dataPath, getDatabase, options) {
    ...
    if (remoteContent) {
        ...
        const filtered = filterRemoteDataByPrefix(remoteData.items, remoteData.comments, remoteData.dependencyEdges || [], remoteData.auditResults || [], projectPrefix);
    ...
    const progressHandler = (evt) => {
        if (isJsonMode)
            return; // avoid polluting JSON output
        try {
...
async function getGitInfo(remote) {
    try {
...
function getLocalDataInfo(filePath) {
...
export default function register(ctx) {
    ...
    const syncCommand = program
        ...
        try {
            ...
            await withFileLock(lockPath, () => performSync(dataPath, utils.getDatabase, {
        ...
        try {
            ...
            if (!reSortNo && typeof db.reSort === 'function') {
                if (reSortSync)
                    db.reSort();
                else
    ...
    syncCommand
        ...
        try {
            const gitTarget = { remote: gitRemote, branch: gitBranch };
