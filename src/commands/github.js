...
import { upsertIssuesFromWorkItems, importIssuesToWorkItems } from '../github-sync.js';
...
import { delegateWorkItem } from '../delegate-helper.js';
export function resolveGithubConfig(options) {
...
function resolveGithubImportCreateNew(options) {
...
export default function register(ctx) {
    ...
    githubCommand
        ...
        const reSortNo = Boolean(options.noReSort) || false;
        const reSortSync = Boolean(options.reSortSync) || false;
        const isJsonMode = utils.isJsonMode();
        const isVerbose = program.opts().verbose;
        // Enable verbose GitHub API logging (to stderr) when --verbose is used
        try {
            setVerboseLogger(isVerbose ? ((m) => console.error(m)) : null);
        }
        catch (_) { }
        let lastProgress = '';
        ...
        const writeProgressMessage = (message, complete = false) => {
        ...
        const progressMode = options.progress;
        ...
        const renderProgress = (progress) => {
            if (progress.phase === 'push') {
                ...
                try {
                    ...
                    if (s)
                        message = `${message} (queue=${s.queueLength} active=${s.active} retries=${s.retryCount} errors=${s.errorCount})`;
                }
                catch (_) { }
            ...
            progressReporter.render(progress);
        };
        try {
            ...
            await withFileLock(lockPath, async () => {
                ...
                const dbForMetadata = typeof db.getAll === 'function' && typeof db.store === 'object' ? db.store : undefined;
                ...
                let _writeLastPushTimestamp = null;
                const forceAll = Boolean(options.all) || Boolean(options.force);
                if (options.force && !options.all) {
                    if (!isJsonMode)
                        console.error('Warning: --force is deprecated and will be removed in a future release. Use --all instead.');
                ...
                if (forceAll) {
                    ...
                    if (!isJsonMode && !options.id)
                        console.log(`Full push (--all): processing all ${items.length} items`);
                    ...
                    try {
                        ...
                        if (!isJsonMode && !options.id) {
                            const parts = [];
                            if (skippedCount > 0)
                                parts.push(`${skippedCount} unchanged since last push`);
                            if (deletedWithoutIssueCount > 0)
                                parts.push(`${deletedWithoutIssueCount} deleted without issue number`);
                        ...
                        const msg = `Pre-filter failed: ${err.message}. Continuing without pre-filter.`;
                        if (!isJsonMode)
                            console.error(msg);
                        ...
                        if (!Number.isNaN(lastMs)) {
                            ...
                            if (additional.length > 0) {
                                ...
                                for (const it of items) {
                                    if (additional.find(a => a.id === it.id))
                                        itemsToProcess.push(it);
                                }
                                // Add comments for additional items
                                for (const c of comments) {
                                    if (additional.find(a => a.id === c.workItemId))
                                        commentsToProcess.push(c);
                                }
                ...
                for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                    ...
                    let batchResult;
                    try {
                        batchResult = await upsertIssuesFromWorkItems(batchItems, batchComments, githubConfig, renderProgress, verboseLog, 
                        ...
                        if (batchError instanceof SecondaryRateLimitError) {
                            ...
                            if (details.stderr)
                                logLine(`github push: stderr: ${details.stderr}`);
                            if (details.stdout)
                                logLine(`github push: stdout: ${details.stdout}`);
                        ...
                        const batchMsg = `Batch ${batchIndex + 1}/${totalBatches} failed: ${batchError.message}`;
                ...
                if (skipUpdateTimestamp) {
                    ...
                    if (!isJsonMode)
                        console.log('Note: last-push timestamp was not updated (--no-update-timestamp)');
                }
                else {
                ...
                if (isJsonMode) {
                    ...
                    if (preFilterSkippedCount > 0 || preFilterDeletedWithoutIssueCount > 0) {
                        const skipParts = [];
                        if (preFilterSkippedCount > 0)
                            skipParts.push(`${preFilterSkippedCount} unchanged since last push`);
                        if (preFilterDeletedWithoutIssueCount > 0)
                            skipParts.push(`${preFilterDeletedWithoutIssueCount} deleted without issue number`);
                        if (result.skipped > 0)
                            skipParts.push(`${result.skipped} up-to-date`);
                        console.log(`    (${skipParts.join(', ')})`);
                    }
                    if (forceAll)
                        console.log('  Note: --all was used; pre-filter was bypassed');
                    if ((result.commentsCreated || 0) > 0 || (result.commentsUpdated || 0) > 0) {
            ...
            logLine(`GitHub sync failed: ${error.message}`);
            output.error(`GitHub sync failed: ${error.message}`, { success: false, error: error.message });
            process.exit(1);
        }
    });
    githubCommand
        ...
        try {
            ...
            if (importedComments.length > 0) {
                ...
                const existingGhIds = new Set(existingComments
                    .filter(c => c.githubCommentId !== undefined)
                    .map(c => c.githubCommentId));
                const newComments = importedComments.filter(c => c.githubCommentId === undefined || !existingGhIds.has(c.githubCommentId));
            ...
            }, mergedItems, logLine, { repoUrl: `https://github.com/${githubConfig.repo}` });
            if (isJsonMode) {
                output.json({
                    success: true,
                    repo: githubConfig.repo,
                ...
                if (isVerbose) {
                    ...
                    displayConflictDetails({
                        ...
                        conflicts: conflictDetails.conflicts,
                        conflictDetails: conflictDetails.conflictDetails,
                    }, mergedItems, { repoUrl: `https://github.com/${githubConfig.repo}` });
                }
            }
            progressReporter.stopHeartbeat();
            ...
            progressReporter.stopHeartbeat();
            logLine(`GitHub import failed: ${error.message}`);
            output.error(`GitHub import failed: ${error.message}`, { success: false, error: error.message });
            process.exit(1);
        }
    });
    githubCommand
            ...
            if (nonClosedChildren.length > 0) {
                ...
                if (isInteractive) {
                    ...
                    const answer = await new Promise(resolve => {
                        rl.question(`Work item ${normalizedId} has ${nonClosedChildren.length} open child item(s). ` +
                            `Only the specified item will be delegated. Continue? (y/N): `, resolve);
                    });
                    rl.close();
        ...
        try {
            const githubConfig = resolveGithubConfig({ repo: options.repo, labelPrefix: options.labelPrefix });
            result = await delegateWorkItem(db, githubConfig, normalizedId, { force: options.force });
        }
        catch (error) {
            const message = `Delegation failed: ${error.message}`;
        ...
        if (!result.success) {
            ...
            if (result.pushed && result.assigned === false && result.issueNumber) {
                const failureMessage = `Failed to assign @copilot to GitHub issue #${result.issueNumber}: ${result.error}. Local state was not updated.`;
