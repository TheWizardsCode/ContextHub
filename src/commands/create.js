...
export default function register(ctx) {
    ...
    program
        ...
        let normalizedStatus = (options.status || 'open');
        ...
        if (auditResultData) {
            refreshed.auditResult = db.getAuditResult(item.id);
            refreshed.audit = { time: auditEntry.time, author: auditEntry.author, text: auditEntry.text, status: auditEntry.status };
        }
        if (utils.isJsonMode()) {
            output.json({
                success: true,
        ...
        try {
            ...
            const reSortNo = ((options.noReSort === true) || (options.reSort === false) || cliNoReSort);
            ...
            if (shouldReSort && !reSortNo && typeof db.reSort === 'function') {
                if (reSortSync)
                    db.reSort();
                else
