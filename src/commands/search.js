...
import { EmbeddingStore, getDefaultEmbedder, createSearch, getEmbeddingStorePath, } from '../lib/search.js';
export default function register(ctx) {
    ...
    program
        ...
        if (options.semantic || options.semanticOnly) {
            ...
            const fusedResults = search.searchSync(query, lexicalInput, semanticMode, { lexicalWeight: 0.5, semanticWeight: 0.5 });
 ...
 */
function triggerSemanticRebuild(db) {
    try {
        const worklogDir = resolveWorklogDir();
        const storePath = getEmbeddingStorePath(worklogDir);
        const store = new EmbeddingStore(storePath);
        ...
        search.reindexAll(items.map((item) => ({
            id: item.id,
            title: item.title ?? '',
