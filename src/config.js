 ...
 */
export function writeInitSemaphore(version) {
 ...
 */
export function readInitSemaphore() {
...
export function loadConfig() {
    ...
    if (fs.existsSync(defaultsPath)) {
        try {
            ...
            config = yaml.load(content, { schema: yaml.CORE_SCHEMA });
        }
        catch (error) {
    ...
    if (fs.existsSync(configPath)) {
        try {
            ...
            const userConfig = yaml.load(content, { schema: yaml.CORE_SCHEMA });
 ...
 */
export function loadConfigRelaxed() {
    let config = null;
    // First, load defaults if they exist
    const defaultsPath = getConfigDefaultsPath();
    if (fs.existsSync(defaultsPath)) {
...
function validateStatusStageConfig(config) {
 ...
 */
export async function initConfig(existingConfig, options) {
    if (existingConfig) {
        ...
        const hasExplicitOptions = Boolean(options?.projectName !== undefined ||
    ...
    let projectName = options?.projectName || existingConfig?.projectName;
    ...
    let prefix = options?.prefix || existingConfig?.prefix;
