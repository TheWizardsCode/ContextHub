...
export class TokenBucketThrottler {
    ...
    taskContext = new AsyncLocalStorage();
     ...
     */
    async waitForIdle(graceMs = 10000, pollInterval = 100) {
        ...
        return new Promise((resolve) => {
            const check = () => {
                try {
                    if (!isBusy())
                        return resolve(true);
                    if (this.clock.now() - start >= graceMs)
    ...
    processQueue() {
        ...
        if (this.tokens < 1) {
            ...
            if (this.debug)
                console.debug(`[throttler] no-tokens tokens=${this.tokens.toFixed(2)} msUntil=${Math.ceil(msUntil)} queue=${this.queue.length} active=${this.active}`);
        ...
        this.active += 1;
        if (this.debug)
            console.debug(`[throttler] dispatch active=${this.active} tokens=${this.tokens.toFixed(2)} queue=${this.queue.length}`);
        // Execute task
        Promise.resolve()
            ...
            task.resolve(res);
            if (this.debug)
                console.debug(`[throttler] complete active=${this.active} tokens=${this.tokens.toFixed(2)} queue=${this.queue.length}`);
            ...
            task.reject(err);
            if (this.debug)
                console.debug(`[throttler] error active=${this.active} tokens=${this.tokens.toFixed(2)} queue=${this.queue.length} err=${String(err?.message ?? err)}`);
            this.processQueue();
        });
 ...
 */
export function makeThrottlerFromEnv(overrides) {
...
//# sourceMappingURL=github-throttler.js.map