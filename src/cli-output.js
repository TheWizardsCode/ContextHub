 ...
 */
export function shouldUseFormattedOutput(enabledByFlag) {
    // If explicitly disabled, don't use formatting
    if (enabledByFlag === false)
        return false;
 ...
 */
export function resolveFormatToMarkdown(formatValue) {
    if (!formatValue)
        return undefined;
    const normalized = formatValue.toLowerCase().trim();
    if (normalized === 'markdown')
        return true;
    if (normalized === 'plain' || normalized === 'text')
        return false;
    if (normalized === 'auto')
        return undefined; // let TTY auto-detect decide
 ...
 */
export function renderCliMarkdown(input, opts) {
    if (!input)
 ...
 * ```
 */
export function createCliOutput(opts) {
    return {
        /**
...
export function resolveMarkdownEnabled(opts) {
    ...
    if (opts.formatAsMarkdown === true)
        return true;
    if (opts.formatAsMarkdown === false)
        return false;
    // Priority 4: config file setting
    if (opts.cliFormatMarkdown === true)
        return true;
    if (opts.cliFormatMarkdown === false)
        return false;
 ...
 */
export function createCliOutputFromCommand(programOpts, configOpts) {
