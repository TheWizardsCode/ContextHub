 ...
 */
export function normalizeSqliteValue(v) {
    if (v === undefined)
        return null;
    if (v === null)
        return null;
 ...
 */
export function normalizeSqliteBindings(values) {
...
export function unescapeText(s) {
    const map = { '\\': '\\', n: '\n', t: '\t', r: '\r' };
    return s.replace(/\\(\\|n|t|r)/g, (_, c) => map[c]);
}
export class SqlitePersistentStore {
    ...
    constructor(dbPath, verbose = false) {
        ...
        if (!fs.existsSync(dir)) {
            try {
                ...
                throw new Error(`Failed to create database directory ${dir}: ${error.message}`);
            }
        }
        ...
        try {
            ...
            throw new Error(`Failed to open database ${dbPath}: ${error.message}`);
        }
        // Initialize schema
        try {
        ...
        if (!isNewDb) {
            ...
            if (existingVersion < SCHEMA_VERSION) {
                ...
                if (!runningInTest) {
                    ...
                    console.warn(`Worklog: database at ${this.dbPath} has schemaVersion=${existingVersion} but the application expects schemaVersion=${SCHEMA_VERSION}. ` +
    ...
    `);
        ...
        if (process.env.WL_DEBUG_SQL_BINDINGS) {
            try {
                ...
                const rawRows = values.map((v, i) => ({ index: i, type: v === null ? 'null' : typeof v, constructor: v && v.constructor ? v.constructor.name : null, value: (() => { try {
                        return v;
                    }
                    catch (_) {
                        return '<unreadable>';
     ...
     */
    batchUpdateSortIndices(orderedItems, gap) {
    ...
    getAllWorkItemsOrderedByHierarchySortIndex() {
     ...
     */
    getAllWorkItemsOrderedByHierarchySortIndexSkipCompleted() {
        const items = this.getAllWorkItems();
     ...
     */
    orderItemsByHierarchySortIndexSkipCompleted(items) {
        const itemMap = new Map();
        const childrenByParent = new Map();
        for (const item of items) {
            itemMap.set(item.id, item);
    ...
    getCommentsForWorkItem(workItemId) {
        ...
        const rows = stmt.all(workItemId);
        return rows.map(row => this.rowToComment(row));
    }
    /**
     * Delete a comment
     ...
     */
    deleteDependencyEdge(fromId, toId) {
     ...
     */
    deleteDependencyEdgesForItem(itemId) {
    ...
    upsertFtsEntry(item) {
        ...
        const deleteFts = this.db.prepare(`DELETE FROM worklog_fts WHERE itemId = ?`);
        const insertFts = this.db.prepare(`
      INSERT INTO worklog_fts (title, description, comments, tags, itemId, status, parentId)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        deleteFts.run(item.id);
        insertFts.run(item.title, item.description, commentText, tagsText, item.id, item.status, item.parentId ?? '');
    }
    /**
     * Remove a work item from the FTS index
     */
    deleteFtsEntry(itemId) {
        if (!this._ftsAvailable)
            return;
    ...
    searchFallback(query, options) {
        ...
        for (const item of items) {
            ...
            for (const term of terms) {
                ...
                if (titleHits > bestFieldScore) {
                    bestFieldScore = titleHits;
                    bestField = 'title';
                }
                if (descHits > bestFieldScore) {
                    bestFieldScore = descHits;
                    bestField = 'description';
                }
                if (commentHits > bestFieldScore) {
                    bestFieldScore = commentHits;
                    bestField = 'comments';
                }
                if (tagHits > bestFieldScore) {
                    bestFieldScore = tagHits;
                    bestField = 'tags';
                }
            }
            if (score > 0) {
     ...
     */
    generateSnippet(text, term, maxLen) {
        if (!text)
            return '';
...
//# sourceMappingURL=persistent-store.js.map