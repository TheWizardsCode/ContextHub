 ...
 */
export function validateFilePaths(items: WorkItem[], intakeStageNames?: string[]): FilePathsFinding[] {
  const findings: FilePathsFinding[] = [];
  const stages = intakeStageNames || DEFAULT_INTAKE_STAGES;

  const intakeItems = items.filter(
    item => stages.includes(item.stage) && item.status !== 'deleted'
  );
    ...
    if (paths.length === 0) {
      ...
      if (!hasSection) {
        findings.push({
          ...
          itemId: item.id,
          message:
            `Missing **Key Files:** section in description. ` +
        ...
        findings.push({
          ...
          itemId: item.id,
          message:
            `**Key Files:** section exists but contains no valid file paths. ` +
 ...
 */
export function applyFilePathsFix(
  finding: FilePathsFinding,
  updateItem: (itemId: string, updates: { description: string }) => boolean,
): boolean {
  if (finding.type !== TYPE_MISSING) {
    return false;
  }

  const proposedFix = finding.proposedFix as Record<string, string> | null;
