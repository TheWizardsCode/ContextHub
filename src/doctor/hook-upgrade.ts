...
/**
 * Canonical v2 pre-push hook content — MUST match the committed
 * `.githooks/pre-push` template (branch-policy enforcement + context-budget
 * gate + worklog sync). Embedded here (rather than read from disk) so the
 * generator works from a dist/ install; `wl init` (src/commands/init.ts)
 * imports this same constant so freshly installed hooks never drift from the
 * committed template (WL-0MT33QNTL006LHQA).
 */
export const PRE_PUSH_HOOK_CONTENT = [
        '#!/bin/sh',
