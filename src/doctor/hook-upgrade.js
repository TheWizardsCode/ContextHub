 ...
 */
function hasWorklogMarker(content) {
 ...
 */
function isHookOutdated(installed, committed) {
    // If they are identical, not outdated
    if (installed.trim() === committed.trim())
        return false;
    // Outdated if missing the safe --git-branch guard
    if (!installed.includes('--git-branch refs/worklog/data'))
        return true;
 ...
 */
export function detectHooksTargetDir() {
    try {
 ...
 */
export function generateCanonicalHookContent(hookName) {
    switch (hookName) {
        case 'pre-push': {
            return [
                '#!/bin/sh',
 ...
 */
export function listOutdatedHooks(githooksDir = '.githooks', hooksDir = '.git/hooks') {
    const info = [];
 ...
 */
export function dryRunHooks(githooksDir, hooksDir) {
 ...
 */
export function upgradeHooks(githooksDir, hooksDir) {
    const outdatedHooks = listOutdatedHooks(githooksDir, hooksDir);
    const upgraded = [];
    const skipped = [];
    let error;
...
//# sourceMappingURL=hook-upgrade.js.map