...
- **Current boot:** 2026-08-22 22:39 (boot 0) — snapd has been running for >9 hours
- **Current version:** 2.76.2 (27710)
- **Current goroutines:** 22 tasks, 78.8M memory, healthy

### Journal Logs (Current Boot)

snapd logs show normal operation with periodic refresh checks and cache cleanups:

```
Aug 22 22:39:32 snapd[6180]: Started snapd.service - Snap Daemon.
Aug 22 22:39:32 snapd[6180]: backends.go:70: AppArmor status: apparmor is enabled
Aug 22 22:39:32 snapd[6180]: standby.go:95: will consider standby after: 5s
```

No error logs or warning signs observed since the incident.
...
### Option B: Disable Watchdog (Fallback)

If upgrading is not possible, disable the systemd watchdog to prevent forced kills:

```bash
sudo systemctl set-property snapd.service WatchdogSec=0
...
**Apply Option A (upgrade to 2.76.3) with Option B (disable watchdog) as a fallback.**

The upgrade is the primary fix because:
1. It addresses the root cause (netlink parser improvements)
2. It includes shutdown reliability fixes
3. It's the latest stable release with security updates

If the upgrade is not possible or the issue recurs after upgrade, apply Option B as a safety net.
