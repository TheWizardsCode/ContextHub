...
it('reproduces triple keypress after reopen', async () => {
  ...
  const agentPane = { serverStatusBox: makeBox(), dialog: makeBox(), textarea: opencodeText, suggestionHint: makeBox(), sendButton: makeBox(), cancelButton: makeBox(), ensureResponsePane: vi.fn(() => makeBox()) } as any;

  const makeDetailBox = () => ({ ...makeBox(), setScroll: vi.fn(), screen, setLine: vi.fn(), setLabel: vi.fn() });
  const layout = { screen, listComponent: { getList: () => makeBox(), getFooter: () => makeBox() }, detailComponent: { getDetail: () => makeDetailBox(), getCopyIdButton: () => makeBox() }, toastComponent: { show: vi.fn() }, overlaysComponent: overlays, dialogsComponent: dialogs, helpMenu: { isVisible: vi.fn(() => false), show: vi.fn(), hide: vi.fn() }, modalDialogs: { selectList: vi.fn(async () => null), editTextarea: vi.fn(async () => null), confirmTextbox: vi.fn(async () => true), forceCleanup: vi.fn() }, agentPane, nextDialog: { overlay: makeBox(), dialog: makeBox(), close: makeBox(), text: makeBox(), options: makeBox() } } as any;
