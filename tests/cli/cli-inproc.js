...
export async function runInProcess(commandLine, timeoutMs = 15000) {
    ...
    process.exit = ((code) => { throw new Error(`__INPROC_EXIT__:${code ?? 0}`); });
    console.log = ((...args) => { out.push(`${args.map(a => String(a)).join(' ')}\n`); });
    console.error = ((...args) => { err.push(`${args.map(a => String(a)).join(' ')}\n`); });
    console.warn = ((...args) => { err.push(`${args.map(a => String(a)).join(' ')}\n`); });
    console.info = ((...args) => { out.push(`${args.map(a => String(a)).join(' ')}\n`); });
    ...
    try {
        ...
        ctx.utils.getDatabase = (prefix) => {
            const db = origGetDatabase(prefix);
            openDatabases.push(db);
            return db;
        };
        // Register built-in commands
        for (const r of builtInCommands)
            r(ctx);
            ...
            try {
                ...
                if (e && e.message === '__INPROC_PARSE_TIMEOUT__') {
                    ...
                    try {
                        ...
                        const waitFn = throttler?.waitForIdle;
...
//# sourceMappingURL=cli-inproc.js.map